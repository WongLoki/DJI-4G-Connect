...
3. 只清理对应硬件已经不存在且仍处于启用状态的残留服务，不触碰 Wi-Fi、VPN、其他网卡或已经禁用的历史残留服务；
4. 如果 ECM 网卡已经工作并获得 `192.168.225.x` 地址，立即退出；
5. 如果 macOS 已识别 ECM 网卡，但对应网络服务被禁用或尚未取得地址，请求管理员授权后启用服务并重新请求 DHCP；
6. 如果网卡仍未就绪，通过 libusb 打开模块的 USB AT 接口；
7. 确认 `AT+QCFG="usbnet"` 为 ECM 模式 `1`，必要时写入；
8. 执行 `AT+CFUN=1,1` 软重启，等待 macOS 重新枚举，并再次修复网络服务与 DHCP；
9. 使用 App 自身的原生通知显示结果，然后退出。
...
当 macOS 网络服务处于禁用状态或需要重新请求 DHCP 时，系统会弹出管理员授权窗口。4G Connect 只对当前受支持模块对应的网络服务执行启用与 DHCP 操作。
